...
contract MockNFT {
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory data) internal {
        _transfer(from, to, tokenId);
        if (to.code.length > 0) {
            try IERC721ReceiverMinimal(to).onERC721Received(msg.sender, from, tokenId, data) returns (bytes4 retval) {
                require(retval == IERC721ReceiverMinimal.onERC721Received.selector, "Invalid receiver");
            } catch {
                revert("Non receiver");
            }
        }
